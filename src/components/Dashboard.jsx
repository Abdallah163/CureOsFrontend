...
export default function Dashboard() {
  ...
  const exportExcel = async () => {
    ...
    try {
      const res = await api.get('/api/analytics/export-excel', { responseType: 'blob' });
      const url = window.URL.createObjectURL(new Blob([res.data]));
      const link = document.createElement('a');
      link.href = url;
      const today = new Date().toLocaleDateString('ar-EG').replace(/\//g, '-');
      link.setAttribute('download', `تحليل_أداء_العيادة_${today}.xlsx`);
    ...
    <div>
      ...
      <div className="flex justify-end gap-2 mb-4 flex-wrap">
        ...
        <button
          ...
          className="flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white px-5 py-2 rounded-lg shadow-md transition-all duration-200 disabled:opacity-50 font-medium"
        >
          {exportingExcel ? (
            <>
