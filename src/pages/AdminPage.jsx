...
export default function AdminPage() {
  ...
  const load = useCallback(
    async (silent = false) => {
      try {
        ...
        p.data.forEach((pay) => {
          defaults[pay._id] =
            pay.plan === "weekly" ? 7 : pay.plan === "4months" ? 120 : 30;
        });
  ...
  const approvePayment = async (id) => {
    ...
    try {
      await api.post(`/api/payment/${id}/approve`, {
        grantedDays: Number(days),
      });
    ...
    <div dir="rtl" className="min-h-screen bg-slate-50">
      ...
      <div className="bg-white shadow-sm p-4 flex justify-between items-center">
        ...
        <div className="flex gap-2 items-center">
          ...
          <Link
            to="/home"
            className="bg-teal-600 text-white px-4 py-1.5 rounded-lg text-sm"
          >
            الرئيسية
          </Link>
          <button
      ...
      <div className="max-w-6xl mx-auto p-6">
        ...
        <div
          id="payments-section"
          className="bg-white rounded-2xl shadow-sm mb-8 overflow-hidden border border-amber-100"
        >
          <div className="p-4 border-b bg-amber-50">
            <div className="flex items-center justify-between">
              <h2 className="font-bold text-amber-900">
                💳 طلبات دفع بانتظار الموافقة ({payments.length})
              </h2>
            ...
            <p className="text-xs text-amber-700 mt-1">
              يُرسل إشعار للبريد وعند ضبط SMTP. يُشعَر الأدمن أيضاً بصوت عند
              وصول طلب جديد.
            </p>
          </div>
          {payments.length === 0 ? (
                  ...
                  >
                    <div className="flex-1 min-w-[200px] text-sm space-y-1">
                      ...
                      <p className="text-gray-500">
                        {p.clinicName} · {p.plan} · {p.amount} ج.م ·{" "}
                        {p.paymentMethod}
                      </p>
                      {hasAddons && (
                    ...
                    {p.receiptData && (
                      ...
                      >
                        <span className="flex items-center justify-center w-full h-full bg-slate-100 text-xs text-teal-700">
                          عرض الإيصال
                        </span>
                      </a>
                    )}
                    <div className="flex flex-col gap-2 items-end">
                      ...
                      <div className="flex gap-2">
                        ...
                        <button
                          ...
                          className="bg-red-100 text-red-700 px-3 py-1.5 rounded-lg text-xs"
                        >
                          رفض
                        </button>
                      </div>
                  ...
                  <tr key={u._id} className="border-t hover:bg-gray-50">
                    ...
                    <td className="p-3">
                      ...
                      {u.addons && u.addons.includes("advertising") && (
                        <span className="mr-1 bg-purple-100 text-purple-700 text-xs px-1.5 py-0.5 rounded-full">
                          📢
                        </span>
                      )}
                    </td>
                    <td className="p-3">
                      <span
                        className={`px-2 py-0.5 rounded-full text-xs ${u.isActive ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
                      >
                        {u.paymentStatus} {u.isActive ? "· نشط" : "· موقوف"}
                      </span>
                    </td>
                    <td className="p-3 text-xs text-gray-500">
                      {u.usage?.visitsCount ?? 0}/{u.usage?.limitVisits ?? "∞"}{" "}
                      · {u.usage?.whatsappCount ?? 0}/
                      {u.usage?.limitWhatsapp ?? "∞"}
                    </td>
...
function Stat({ label, value, color }) {
  ...
  return (
    <div
      className={`bg-white p-4 rounded-xl shadow border-r-4 ${colors[color]}`}
    >
