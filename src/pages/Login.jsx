...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!res.data?.token) {
        setError(
          "لم يصل رمز الدخول من الخادم. أعد المحاولة أو تواصل مع الدعم.",
        );
        return;
      }
      ...
      if (res.data.warning) {
        sessionStorage.setItem(
          "subscriptionWarning",
          JSON.stringify(res.data.warning),
        );
      }
      ...
      if (err.response?.status === 402 && data?.requirePayment) {
        setError(
          data.message || "انتهت الفترة المجانية. سيتم توجيهك لصفحة الدفع.",
        );
        setTimeout(() => {
          navigate("/payment", {
            state: {
      ...
      if (!err.response) {
        setError(
          "تعذر الاتصال بالخادم. تحقق من الإنترنت أو أعد بناء الموقع بـ REACT_APP_API_URL.",
        );
      } else {
    ...
    <div dir="rtl" className="auth-page">
      <div className="auth-card max-w-md">
        ...
        <div className="mt-6 pt-5 border-t border-slate-100 text-center">
          <Link
            to="/register"
            className="block w-full border-2 border-teal-600 text-teal-700 py-2.5 rounded-xl font-semibold text-sm hover:bg-teal-600 hover:text-white transition mb-2"
          >
