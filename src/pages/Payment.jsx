...
const PLANS = [
  ...
  },
  {
    ...
    price: 1500,
    duration: 'مرّة واحدة',
    durationNote: 'تفعيل لحملة دعائية واحدة',
    color: 'purple',
    highlight: false,
    description: 'حملة دعائية لمرضيك السابقين عبر واتساب',
    features: [
      { text: 'رفع ملفات Excel',               ok: false },
      { text: 'لوحة تحكم أساسية',               ok: false },
      { text: 'زيارات عيادة',                   ok: false },
      { text: 'مرضى غير محدودين',               ok: false },
      { text: 'تقارير إيرادات',                 ok: false },
      { text: 'تصدير تقرير PDF',                ok: false },
      { text: 'تذكير المرضى عبر واتساب',        ok: true, note: '40 رسالة دعائية فقط' },
      { text: 'قياس رضا المرضى',                ok: false },
      { text: 'الإجراءات التصحيحية الذكية',      ok: false },
      { text: 'تقارير متقدمة ومقارنات',         ok: false },
      { text: 'دعم فني بالأولوية',              ok: false },
    ],
...
export default function Payment() {
  ...
  const [selectedPlan, setSelectedPlan]     = useState(location.state?.fromRegister ? 'weekly' : null);
  ...
  const [showRegForm, setShowRegForm]       = useState(!!location.state?.fromRegister);
  ...
  const handleSubmit = async (e) => {
    ...
    if (showRegForm && (!regData.name || !regData.email || !regData.password)) {
      alert('يرجى إكمال بيانات التسجيل'); return;
    }
    ...
    try {
      ...
      formData.append('amount', plan?.price || 500);
    ...
    <div dir="rtl" className="min-h-screen bg-gradient-to-br from-green-50 via-white to-emerald-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-xl p-10 max-w-md w-full text-center border border-green-100">
        ...
        <div className="flex flex-col gap-2">
          <Link to="/login" className="block w-full bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-xl font-semibold transition">
            {showRegForm ? 'تسجيل الدخول بعد التفعيل' : 'العودة للوحة التحكم'}
          </Link>
        </div>
      </div>
    </div>
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50">
      ...
      <div className="bg-gradient-to-l from-indigo-700 to-blue-600 text-white py-10 px-4 text-center">
        <p className="text-blue-200 text-sm mb-2 font-medium">🏥 للعيادات المتخصصة</p>
        <h1 className="text-3xl font-extrabold mb-3">اشترك لتفعيل حسابك</h1>
        <p className="text-blue-100 text-sm max-w-md mx-auto">
          ادفع الحد الأدنى 500 ج.م لإنشاء حسابك والاستفادة من خدمات التحليل
        </p>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 py-10">
        ...
        {showRegForm && (
          <div className="bg-blue-50 border border-blue-200 rounded-2xl p-6 mb-8">
            <h2 className="font-bold text-gray-800 text-lg mb-4 flex items-center gap-2">
              <span className="w-7 h-7 bg-blue-600 text-white rounded-full text-sm flex items-center justify-center font-bold">📝</span>
              بيانات الحساب الجديد
            </h2>
            <p className="text-sm text-gray-500 mb-4">أدخل بياناتك مرة واحدة. سيتم إنشاء حسابك تلقائياً بعد تأكيد الدفع.</p>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div>
                ...
                <input type="text" value={regData.name}
                  ...
                  className="w-full p-2.5 border border-blue-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-300 bg-white"
                  placeholder="د. أحمد محمد" required />
              </div>
              <div>
                <label className="text-xs text-gray-500 mb-1 block">اسم العيادة</label>
              ...
              <div>
                <label className="text-xs text-gray-500 mb-1 block">رقم واتساب</label>
        ...
        {method && plan && (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-5">
              ...
              <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xl font-extrabold text-gray-800">{plan.price.toLocaleString()} <span className="text-sm font-normal text-gray-400">ج.م</span></p>
                </div>
            ...
            >
              {submitting ? 'جارٍ الإرسال...' : (showRegForm ? 'اشتراك وإنشاء حساب ←' : 'إرسال طلب التفعيل ←')}
            </button>
          </div>
        )}
